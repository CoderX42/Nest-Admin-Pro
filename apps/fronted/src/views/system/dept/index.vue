<template>
  <div class="page-container">
    <div class="search-bar">
      <el-input v-model="queryParams.name" :placeholder="t('system.dept.placeholderName')" style="width: 220px" clearable @clear="loadData" />
      <el-select v-model="queryParams.status" :placeholder="t('common.field.status')" style="width: 140px" clearable @clear="loadData"
      >
    ...
    <div class="toolbar">
      ...
      >{{ t('system.dept.addDept') }}</el-button
      >
    </div>
    ...
    <el-table :data="tableData" v-loading="loading" row-key="id" default-expand-all>
      ...
      <el-table-column :label="t('common.field.actions')" width="200" fixed="right">
        <template #default="{ row }">
          ...
          >
          <el-button size="small" type="danger" :icon="Delete" @click="handleDelete(row)" :disabled="row.children?.length > 0"
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? t('system.dept.editDept') : t('system.dept.addDept')" width="500px">
...
const rules = { name: [{ required: true, message: t('system.dept.placeholderName'), trigger: 'blur' }] };
...
const loadData = async () => {
  ...
  try {
    ...
    tableData.value = res;
    deptTree.value = [{ id: 0, name: t('system.dept.rootDept'), children: res }];
  } catch (e) {
