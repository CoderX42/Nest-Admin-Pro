...
export class NoticeService {
  ...
  async create(dto: CreateNoticeDto) {
    return this.prisma.sysNotice.create({
      data: { title: dto.title, content: dto.content, type: dto.type ?? 1, status: dto.status ?? 1, publishTime: dto.publishTime },
    });
  }
  ...
  async update(dto: UpdateNoticeDto) {
    ...
    if (!notice) throw new NotFoundException('Notice not found');
    return this.prisma.sysNotice.update({ where: { id: dto.id }, data: { title: dto.title, content: dto.content, type: dto.type, status: dto.status, publishTime: dto.publishTime } });
  }

  async remove(id: number) {
    await this.prisma.sysNotice.delete({ where: { id } });
    return { success: true };
  }
}